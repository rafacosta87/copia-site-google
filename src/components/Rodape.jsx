...
function Rodape() {
    ...
    const itemMenuClass = `group flex justify-between items-center w-full py-[4px] px-4 transition-colors ${temaEscuro ? "hover:bg-[#3c4043] text-white" : "hover:bg-[#f1f3f4] text-[#1f1f1f]"
        }`;

    return (
        <footer
            className={`w-full flex flex-col text-[14px] ${temaEscuro ? "bg-[#171717] text-[#e8eaed]" : "bg-[#f2f2f2] text-[#1f1f1f]"
                }`}
        >
            {/* Seção Localização (Brasil) */}
            <div className={`px-[30px] py-[13px] border-b ${temaEscuro ? "border-[#444746]" : "border-[#dadce0]"
                }`}>
                Brasil
            </div>
            ...
            <div className="flex flex-wrap w-full justify-between max-[816px]:justify-center"> 
                ...
                <div className="flex flex-wrap justify-evenly gap-x-7 px-[30px] py-[13px]">
                    ...
                    <div className="relative" ref={searchbarRef}>
                        ...
                        {/* Menu de Configurações (Subindo), perguntar sobre bottom-full */}
                        {focused && (
                            <div className={`absolute bottom-full mb-3 right-0 min-w-[210px] py-1.5 rounded-lg z-50 overflow-hidden ${temaEscuro ? "bg-[#1f1f1f] shadow-[0_2px_10px_0_rgba(0,0,0,0.2)]" : "bg-white border border-[#dadce0] shadow-[0_2px_10px_0_rgba(0,0,0,0.2)]"
                                }`}>
